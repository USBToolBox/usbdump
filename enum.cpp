...
)
{
    ...
    BOOL                             success;
    std::vector<PUSBHOSTCONTROLLERINFO> hostControllers;
         ...
         index++)
    {
        ...
        {
            PUSBHOSTCONTROLLERINFO info = EnumerateHostController(hTreeParent,
                                    hHCDev,
...
EnumerateHostController (
    HTREEITEM                hTreeParent,
    HANDLE                   hHCDev,    _Inout_ PWCHAR            leafName,
    _In_    HANDLE           deviceInfo,
    _In_    PSP_DEVINFO_DATA deviceInfoData
)
{
    ...
    if (rootHubName != NULL)
    {
        ...
        if (SUCCEEDED(hr))
        {
            PUSBROOTHUBINFO info = EnumerateHub(hHCItem,
                         rootHubName,
                         cbHubName,
                         NULL,       // ConnectionInfo
    ...
    )
{
    ...
    //
    hr = StringCbLength(L"\\\\.\\", MAX_DEVICE_PROP, &cchHeader);
    if (FAILED(hr))
    {
        goto EnumerateHubError;
    }
    ...
    //
    hr = StringCchCopyN(deviceName, cchFullHubName, L"\\\\.\\", cchHeader);
    if (FAILED(hr))
    {
        goto EnumerateHubError;
    }
    ...
    if (ConnectionInfo)
    {
        StringCchPrintf(leafName, dwSizeOfLeafName, L"[Port%d] ", ConnectionInfo->ConnectionIndex);
    ...
    for (index = 1; index <= NumPorts; index++)
    {
        ...
        if (connectionInfoEx->DeviceIsHub)
        {
            ...
            if (extHubName != NULL)
            {
                ...
                if (SUCCEEDED(hr))
                {
                    info = static_cast<PUSBEXTERNALHUBINFO>(EnumerateHub(hTreeParent, //hPortItem,
                            extHubName,
                            cbHubName,
                            connectionInfoEx,
                            connectionInfoExV2,
        ...
        else
        {
            ...
            StringCchPrintf(leafName, sizeof(leafName), L"[Port%d] ", index);
    ...
    PUSBHOSTCONTROLLERINFO hcInfo)
{
    ...
    for ( ; nPowerState <= WdmUsbPowerSystemShutdown; nIndex++, nPowerState++) 
    {
        ...
        UsbPowerInfoRequest.PowerInformation.SystemState = static_cast<WDMUSB_POWER_STATE>(nPowerState);
    ...
    )
{
    ...
    while (pEntry != &pList->ListHead)
    {
        pNode = CONTAINING_RECORD(pEntry,
                                  DEVICE_INFO_NODE,
                                  ListEntry);
        if (pNode && _wcsicmp(DriverKeyName, pNode->DeviceDriverName) == 0)
        {
            return pNode;
        }
